...
class BiEncoderTrainer(object):
    ...
    def validate_and_save(self, epoch: int, iteration: int, scheduler):
        ...
        if not cfg.dev_datasets:
            ...
            # validation_loss = self.validate_average_rank_qctx()
            # else:
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            loss, correct_cnt, correct_qcnt = _do_biencoder_fwd_pass(
                self.biencoder,
                biencoder_input,
                self.tensorizer,
                cfg,
        ...
        logger.info(
            "NLL Validation: loss = %f. correct prediction ratio  %d/%d ~  %f correct q_prediction ratio  %d/%d ~  %f",
            total_loss,
    ...
    def validate_average_rank_qctx(self) -> float:
        """
        Validates biencoder model using each question's gold passage's rank across the set of passages from the dataset.
        It generates vectors for specified amount of negative passages from each question (see --val_av_rank_xxx params)
        and stores them in RAM as well as question vectors.
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            # split contexts batch into sub batches since it is supposed to be too large to be processed in one batch
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):
                keep_ids = [i for i in range(0, len(biencoder_input.q_ids), 2)]
                q_ids, q_segments = (
                    (biencoder_input.q_ids[keep_ids], biencoder_input.q_segments[keep_ids]) if j == 0 else (None, None)
                )
        ...
        av_rank = float(rank / q_num)
        logger.info("Av.rank validation: average rank %s, total questions qctx=%d", av_rank, q_num)
        return av_rank
...
def _calc_loss(
    ...
    if distributed_world_size > 1:
        ...
        for i, item in enumerate(global_question_ctx_vectors):
            ...
            # ours
            if ctx_vectors.size(0) != qctx_vectors.size(0):
                print("Dif_size!")
    ...
    loss, is_correct, is_correct_q = loss_function.calc(
        global_q_vector,
        global_ctxs_vector,
    ...
    return loss, is_correct, is_correct_q
...
def _do_biencoder_fwd_pass(
    ...
    loss, is_correct, is_correct_q = _calc_loss(
        cfg,
        loss_function,
        local_q_vector,
        local_ctx_vectors,
...
def main(cfg: DictConfig):
    ...
    if cfg.train_datasets and len(cfg.train_datasets) > 0:
        ...
        # trainer.validate_average_rank_qctx()
    else:
