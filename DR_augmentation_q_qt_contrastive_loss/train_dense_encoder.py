...
class BiEncoderTrainer(object):
    ...
    def run_train(self):
        ...
        # warmup_steps = total_updates * 0.1
    ...
    def validate_nll(self) -> float:
        ...
        # ours
        total_correct_predictions_q = 0
        total_correct_predictions_typos = 0
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            loss, correct_cnt, correct_qcnt, correct_cnt_typos = _do_biencoder_fwd_pass(
                self.biencoder,
                biencoder_input,
                self.tensorizer,
                cfg,
            ...
            # ours
            total_correct_predictions_q += correct_qcnt
            total_correct_predictions_typos += correct_cnt_typos
            batches += 1
        ...
        # ours
        correct_ratio_q = float(total_correct_predictions_q / total_samples)
        correct_ratio_typos = float(total_correct_predictions_typos / total_samples)
        logger.info(
            "NLL Validation: loss = %f. correct prediction ratio  %d/%d ~  %f correct q_prediction ratio  %d/%d ~  %f correct q_prediction ratio  %d/%d ~  %f",
            total_loss,
            total_correct_predictions,
            total_samples,
            correct_ratio,
            total_correct_predictions_q,
            total_samples,
            correct_ratio_q,
            total_correct_predictions_typos,
            total_samples,
            correct_ratio_typos,
        )
        return total_loss
    ...
    def _train_epoch(
        ...
        # ours
        epoch_correct_predictions_q = 0
        epoch_correct_predictions_typos = 0
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            # ours
            epoch_correct_predictions_q += correct_qcnt
            epoch_correct_predictions_typos += correct_cnt_typos
        ...
        logger.info("epoch total correct predictions typos=%d", epoch_correct_predictions_typos)
        logger.info("epoch total correct predictions q=%d", epoch_correct_predictions_q)
...
def _calc_loss(
    ...
    if distributed_world_size > 1:
        ...
        # ours
        qctx_vector_to_send = torch.empty_like(local_qctx_vectors).cpu().copy_(local_qctx_vectors).detach_()

        global_question_ctx_vectors = all_gather_list(
            [
                q_vector_to_send,
                ctx_vector_to_send,
                qctx_vector_to_send,  # ours
                local_positive_idxs,
        ...
        for i, item in enumerate(global_question_ctx_vectors):
            q_vector, ctx_vectors, qctx_vectors, positive_idx, hard_negatives_idxs = item

            if i != cfg.local_rank:
                global_q_vector.append(q_vector.to(local_q_vector.device))
                global_ctxs_vector.append(ctx_vectors.to(local_q_vector.device))
                # ours
                global_qctxs_vector.append(qctx_vectors.to(local_q_vector.device))
                ...
                # ours
                global_qctxs_vector.append(local_qctx_vectors)
        ...
        # ours
        global_qctxs_vector = torch.cat(global_qctxs_vector, dim=0)

    else:
        global_q_vector = local_q_vector
        global_ctxs_vector = local_ctx_vectors
        # ours
        global_qctxs_vector = local_qctx_vectors
        positive_idx_per_question = local_positive_idxs
        hard_negatives_per_question = local_hard_negatives_idxs

    loss, is_correct, is_correct_q, is_correct_q_typos = loss_function.calc(
        global_q_vector,
        global_ctxs_vector,
        global_qctxs_vector,  # ours
        positive_idx_per_question,
        hard_negatives_per_question,
        loss_scale=loss_scale,
    )

    return loss, is_correct, is_correct_q, is_correct_q_typos
...
def _do_biencoder_fwd_pass(
    ...
    # ours
    qctx_attn_mask = tensorizer.get_attn_mask(input.q_ids)
    ...
    local_q_vector, local_ctx_vectors, local_qctx_vectors = model_out

    loss_function = BiEncoderNllLoss()

    loss, is_correct, is_correct_q, is_correct_q_typos = _calc_loss(
        cfg,
        loss_function,
        local_q_vector,
        local_ctx_vectors,
    ...
    # ours
    is_correct_q = is_correct_q.sum().item()
    is_correct_q_typos = is_correct_q_typos.sum().item()
