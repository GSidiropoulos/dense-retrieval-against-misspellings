...
class BiEncoder(nn.Module):
    ...
    def forward(
        ...
        # ours
        if q_ids != None:
            # qctx_encoder = self.question_model if encoder_type is None or encoder_type == "question" else self.ctx_model
    ...
    def create_biencoder_input2(
        ...
        # ours
        q_tensors = []
        q_positive_augm = [aug(s.query) for s in samples]
        for sample_id, sample in enumerate(samples):

            positive_ctx = sample.positive_passages[0]
            # ours if statement helps for the case where we dont want to create
            # online typos for the dev set
            positive_q = q_positive_augm[sample_id] if len(sample.positive_questions) == 0 else \
                sample.positive_questions[0]
...
class BiEncoderNllLoss(object):
    def calc(
        ...
        # ours
        # (q, q_typos, q_negative)
        scores_q = self.get_scores(q_vectors, qctx_vectors)
        # (q_typos, ctx)
        scores_q_typos = self.get_scores(qctx_vectors[positive_idx_per_question], ctx_vectors)
        ...
        if len(q_vectors.size()) > 1:
            ...
            scores_q_typos = scores_q_typos.view(q_num, -1)

        softmax_scores = F.log_softmax(scores, dim=1)

        # ours
        softmax_scores_q = F.log_softmax(scores_q, dim=1)
        softmax_scores_q_typos = F.log_softmax(scores_q_typos, dim=1)
        ...
        loss_q_typos = F.nll_loss(
            softmax_scores_q_typos,
            torch.tensor(positive_idx_per_question).to(softmax_scores_q_typos.device),
            reduction="mean",
        )
        ...
        max_score_q_typos, max_idxs_q_typos = torch.max(softmax_scores_q_typos, 1)
        correct_predictions_count_q_typos = (
                max_idxs_q_typos == torch.tensor(positive_idx_per_question).to(max_idxs_q_typos.device)).sum()

        if loss_scale:
            loss.mul_(loss_scale)
            # ours
            loss_q.mul_(loss_scale)
            loss_q_typos.mul_(loss_scale)

        # ours
        loss_combined = loss + loss_q + loss_q_typos

        return loss_combined, correct_predictions_count, correct_predictions_count_q, correct_predictions_count_q_typos
