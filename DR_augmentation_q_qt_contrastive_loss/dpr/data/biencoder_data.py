...
class JsonQADataset(Dataset):
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        ...
        positive_q = json_sample["positive_q"]
        hard_negative_q = json_sample["hard_negative_q"] if "hard_negative_q" in json_sample else []
        ...
        r.positive_questions = [self._process_query(ctx["text"]) for ctx in positive_q] if "dev" in json_sample else []
        r.hard_negative_questions = [self._process_query(ctx["text"]) for ctx in hard_negative_q]
...
def split_tables_to_chunks(
    ...
    chunks = []
    chunk_id = 0
    for i, t in enumerate(tables_as_dicts):        assert split_type == "type1"
